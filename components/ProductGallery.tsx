...
export default function ProductGallery({ slug, name }: { slug: string; name: string }) {
  ...
  if (!["velomate", "history", "melodymap"].includes(slug)) return null;
  return <section className={`product-gallery product-gallery--${slug}`} aria-label={lang === "tr" ? "Uygulama ekranları" : "App screenshots"}>
    ...
    <div className="product-gallery__screens">{[0,1,2].map(i=><figure key={i}><Image src={`/films/${slug}-${i}.webp`} alt={`${name} — ${lang === "tr" ? "ekran" : "screen"} ${i+1}`} width={960} height={2078} sizes="(max-width: 700px) 65vw, 27vw"/><figcaption>0{i+1} / {name}</figcaption></figure>)}</div>
  </section>;
}
