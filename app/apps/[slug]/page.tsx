    ...
    <main className={styles.main}>
      <section className={styles.hero}>
        ...
        <div><span className={styles.kicker}>{app.category}</span><h1>{app.name}</h1><p>{app.description}</p>
          <div className={styles.meta}><span>{app.status}</span>{app.version && <span>Sürüm {app.version}</span>}<span>iPhone ve iPad</span></div>
          <div className={styles.actions}>{app.appStoreUrl && <a href={app.appStoreUrl} target="_blank" rel="noopener noreferrer">App Store’da görüntüle ↗</a>}<Link className={styles.secondary} href={`/apps/${app.slug}/support`}>Destek</Link><Link className={styles.secondary} href={`/apps/${app.slug}/privacy`}>Gizlilik</Link>{app.githubUrl && <a className={styles.secondary} href={app.githubUrl} target="_blank" rel="noopener noreferrer">GitHub ↗</a>}</div>
        </div>
      </section>
